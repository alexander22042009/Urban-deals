(function () {
    ...
    function updateSummary(cart) {
        ...
        document.querySelector("[data-summary-items]").textContent = String(itemsCount);
        document.querySelector("[data-summary-subtotal]").textContent = money(subtotal);
        document.querySelector("[data-summary-discount]").textContent = `- ${money(discount)}`;

        const voucherEl = document.querySelector("[data-summary-voucher]");
        if (voucherEl) {
            voucherEl.textContent = voucher ? `-${voucher.percent}% (${voucher.id})` : "—";
        }

        document.querySelector("[data-summary-total]").textContent = money(finalTotal);
    }
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        if (typeof window.Store.getVouchers !== "function" ||
            typeof window.Store.getAppliedVoucher !== "function" ||
            typeof window.Store.setAppliedVoucher !== "function") {
            console.error("Voucher API missing in Store. Add getVouchers/getAppliedVoucher/setAppliedVoucher to assets/js/store.js");
            return;
        }
